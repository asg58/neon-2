...
function GlassNeonText({ text, color, intensity, font, depth, envMap, position, rotation, onClick }) {
  ...
  return (
    <mesh ref={mesh} position={position} rotation={rotation} onClick={onClick} castShadow>
...
export default function App() {
  ...
  const [textPosition, setTextPosition] = useState([-4.5, 1.5, -3]); // Beginpositie op een muur
  const [textRotation, setTextRotation] = useState([0, Math.PI / 2, 0]); // Beginrotatie voor muur
  ...
  const handleTextSelect = () => {
    alert("Tekst geselecteerd! Pas eigenschappen hier aan.");
  };
  ...
  return (
    <>
      ...
      >
        ...
        {/* Laad het .glb showroommodel met aanpasbare lichtintensiteit en kleur */}
        <ShowroomModel lightIntensity={lightIntensity} lightColor={lightColor} />

        {/* Plaats de neon-tekst op een muur */}
        <GlassNeonText
          text={text}
          color={color}
          intensity={10}
          font={font}
          depth={depth}
          position={textPosition} // Gebruik beginpositie
          rotation={textRotation} // Gebruik beginrotatie
          onClick={handleTextSelect} // Maak de tekst selecteerbaar
        />
