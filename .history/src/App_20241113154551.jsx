...
function GlassNeonText({ text, color, intensity, font, depth, envMap, position, rotation, onClick }) {
  ...
  const bind = useDrag(({ offset: [x, y], memo = currentPosition }) => {
    const newPosX = memo[0] + x / 10;
    const newPosY = memo[1] - y / 10;
    const clampedX = Math.max(-5, Math.min(5, newPosX));
    const clampedY = Math.max(-5, Math.min(5, newPosY));
  ...
  return (
    <>
      <Canvas dpr={Math.min(window.devicePixelRatio, 2)} style={{ width: '100vw', height: '100vh' }} shadows>
        ...
        <OrbitControls ref={orbitRef} enablePan={true} enableZoom={true} enableRotate={true} /> {/* Zoom & Rotate geactiveerd */}
      ...
      <div style={{ position: 'absolute', top: 20, left: 20, color: 'white', backgroundColor: 'rgba(0,0,0,0.5)', padding: '10px', borderRadius: '8px' }}>
        ...
        <div>
          <h4>Text Position</h4>
          <button onClick={() => moveText(0, -0.1)}>Left</button>
          <button onClick={() => moveText(0, 0.1)}>Right</button>
          <button onClick={() => moveText(1, 0.1)}>Up</button>
          <button onClick={() => moveText(1, -0.1)}>Down</button>
          <button onClick={() => moveText(2, -0.1)}>Forward</button>
          <button onClick={() => moveText(2, 0.1)}>Backward</button>
        </div>
        <div>
