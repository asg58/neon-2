...
function GlassNeonText({ text, color, intensity }) {
  const mesh = useRef();
  ...
  return (
    <>
      <CubeCamera
        args={[0.1, 10, cubeRenderTarget]}
        position={[0, 1, 0]}
        onUpdate={(camera) => camera.update(mesh.current.parent)}
      />
      <mesh ref={mesh}>
        <meshPhysicalMaterial
          ...
          reflectivity={1}
          envMap={cubeRenderTarget.texture} // Omgevingsreflectie
        />
      </mesh>
    </>
  );
  ...
  return (
    <>
      <Canvas>
        ...
        <OrbitControls />
        <GlassNeonText text="Neon Text" color="#00ff00" intensity={5} />
