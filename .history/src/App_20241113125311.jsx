  ...
  return (
    <mesh ref={mesh}>
      <meshPhysicalMaterial
        color={color}
        emissive={color}
        emissiveIntensity={intensity} // Sterkere gloed
        roughness={0.2}
        metalness={0.1}
        transmission={0.9} // Transparantie voor diepte
        thickness={0.3} // Dikte van de glaslaag
        ior={1.4} // Brekingsindex
        reflectivity={0.8}
...
function SceneWithReflection({ color }) {
  ...
  return (
    <>
      ...
      <GlassNeonText text="Neon Text" color={color} intensity={10} envMap={cubeRenderTarget.current.texture} />
    </>
  );
}
...
export default function App() {
  ...
  const [color, setColor] = useState("#39FF14"); // Standaard neon-groene kleur
  ...
  const handleColorChange = (e) => {
    setColor(e.target.value); // Update de kleur state
  };
  ...
  return (
    <>
      ...
      >
        ...
        <OrbitControls />
        <SceneWithReflection color={color} />
        <BloomEffect bloomSettings={bloomSettings} />
      </Canvas>
      
      {/* Instellingen voor gloed en kleurkiezer */}
