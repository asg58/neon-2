...
export default function ControlsPanel({
  ...
  return (
    <div style={{ position: 'absolute', top: 20, left: 20, color: 'white', backgroundColor: 'rgba(0,0,0,0.5)', padding: '10px', borderRadius: '8px' }}>
      ...
      <div>
        <h4>Dimensions (cm)</h4>
        <p>Width: {dimensions.width ? `${dimensions.width.toFixed(2)} cm` : 'Loading...'}</p>
        <p>Height: {dimensions.height ? `${dimensions.height.toFixed(2)} cm` : 'Loading...'}</p>
        <p>Depth: {dimensions.depth ? `${dimensions.depth.toFixed(2)} cm` : 'Loading...'}</p>
      </div>
    </div>
  );
}
