...
export default function App() {
  ...
  const [envMap, setEnvMap] = useState(null); // Voeg omgevingsreflectie toe
  ...
  // Laad de omgevingsteksturen
  useEffect(() => {
    const textureLoader = new THREE.CubeTextureLoader();
    textureLoader.load(
      [
        '/path/to/px.jpg', '/path/to/nx.jpg', // Voeg hier je path naar de cube textures in
        '/path/to/py.jpg', '/path/to/ny.jpg',
        '/path/to/pz.jpg', '/path/to/nz.jpg'
      ],
      (loadedTexture) => {
        setEnvMap(loadedTexture);
      }
    );
  }, []);
  ...
  return (
    <>
      ...
      >
        ...
        {/* Simple Wall Panel - Rotatie aangepast voor verticaal paneel */}
        <mesh position={[0, 0, -5]} rotation={[0, 0, 0]}>
          <planeGeometry args={[10, 6]} />
          <meshStandardMaterial color="gray" />
        </mesh>
        
        {/* Text Object with improved quality */}
        <GlassNeonText
          text={text}
          color={color}
          intensity={10}
          ...
          envMap={envMap} // Voeg hier een environment map toe
        />
