...
function GlassNeonText({ text, color, intensity, font, envMap, depth }) {
  const mesh = useRef();
  ...
  useEffect(() => {
    if (mesh.current && font) {
      const geometry = new TextGeometry(text, {
        font: font,
        size: 1,
        depth: depth, // Gebruik de dynamische diepte
      });
      geometry.center();
...
export default function App() {
  ...
  const [depth, setDepth] = useState(0.1); // Diepte state toevoegen
  const fontFiles = [
  ...
  const handleDepthChange = (e) => {
    setDepth(parseFloat(e.target.value)); // Update de diepte op basis van invoer
  };
