...
import BloomEffect from './components/BloomEffect'; // Alleen nodig als je nog wat extra glow wilt
...
export default function App() {
  ...
  const [color, setColor] = useState("#39FF14"); // Neon groen
  const [text, setText] = useState("Neon Text");
  const { fonts, error } = useFetchFonts(); // Haal fonts op van de server

  const [selectedFont, setSelectedFont] = useState(""); // Standaard naar lege string
  ...
  return (
    <>
      <Canvas style={{ width: '100vw', height: '100vh' }}>
        ...
        <OrbitControls />
        <NeonText text={text} color={color} intensity={2} font={font} />
        
        {/* Je kunt BloomEffect nog toevoegen als je wat extra glow wilt */}
